<template>
  <div class="min-h-screen bg-stone-950 flex items-center justify-center px-4">
    <div class="fixed inset-0 pointer-events-none overflow-hidden">
      <div class="absolute -top-40 -right-40 w-96 h-96 rounded-full opacity-10" style="background:radial-gradient(circle,#dc2626,transparent)"></div>
      <div class="absolute -bottom-40 -left-40 w-96 h-96 rounded-full opacity-10" style="background:radial-gradient(circle,#ea580c,transparent)"></div>
    </div>
    ...
    <div class="w-full max-w-sm relative">
      <div class="text-center mb-8">
        ...
        <p class="text-stone-500 text-sm mt-1">Start your fitness journey.</p>
      </div>

      <div class="bg-stone-900 border border-stone-800/60 rounded-2xl p-6">
        <h2 class="font-display text-2xl tracking-wider text-white mb-5">CREATE ACCOUNT</h2>
        ...
        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-xs font-black text-stone-400 uppercase tracking-widest mb-2">Nama Lengkap</label>
            <input v-model="form.name" type="text" required placeholder="John Doe" minlength="2"
          ...
          <div>
            ...
            <div class="relative">
              <input v-model="form.password" :type="showPw ? 'text' : 'password'" required placeholder="Min. 6 karakter" minlength="6"
          ...
          <div>
            <label class="block text-xs font-black text-stone-400 uppercase tracking-widest mb-2">Konfirmasi Password</label>
            <input v-model="form.confirm" :type="showPw ? 'text' : 'password'" required placeholder="Ulangi password"
              class="w-full bg-stone-800/60 border border-stone-700/50 rounded-xl px-4 py-2.5 text-sm text-white placeholder-stone-600 focus:outline-none focus:border-ember-500 transition-all" />
          </div>
          <button type="submit" :disabled="loading"
            class="w-full bg-fire-grad text-white font-black py-3 rounded-xl shadow-fire hover:shadow-fire-lg transition-all disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        <p class="text-center text-sm text-stone-500 mt-5">
          Sudah punya akun?
          <RouterLink to="/login" class="text-ember-400 font-bold hover:text-ember-300 transition-colors">Masuk</RouterLink>
        </p>
      </div>
    </div>
  </div>
...
async function submit() {
  error.value = ''
  if (form.value.password !== form.value.confirm) { error.value = 'Password tidak cocok.'; return }
  loading.value = true
  try {
